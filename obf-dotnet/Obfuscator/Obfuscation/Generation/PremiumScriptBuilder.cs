...
namespace Obfuscator.Obfuscation.Generation
{
	public class PremiumScriptBuilder
	{
		...
		private void GenerateDeserializer()
		{
			...
			}.Shuffle()) + "\r\n\r\n    }); \r\n\r\nend; " + (ObfuscationSettings.MaximumSecurityEnabled ? "PSU_MAX_SECURITY_END()" : "") + "\n";
		...
		public string BuildScript(string Location)
		{
			...
			if (!ObfuscationSettings.CompressedOutput)
			{
				...
				if (ObfuscationSettings.MaximumSecurityEnabled)
				{
					...
					foreach (KeyValuePair<long, NumberEquation> NumberEquationPair2 in NumberEquations)
					{
			...
			foreach (KeyValuePair<long, NumberEquation> NumberEquationPair in ObfuscationContext.NumberEquations)
			{
			...
			if (ObfuscationSettings.DebugMode)
			{
			...
			string tempsource = Source.Replace("PSU_BYTECODE", ObfuscationContext.ByteCode).Replace("PSU_FORMAT_TABLE", ObfuscationContext.FormatTable);
